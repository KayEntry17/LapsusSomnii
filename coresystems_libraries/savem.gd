...
func save():
	...
	return save_dict
func save_game():
	var save_game = FileAccess.open("user://sv.save", FileAccess.WRITE)
	var json_string=JSON.stringify(save())
	save_game.store_line(json_string)
# Called every frame. 'delta' is the elapsed time since the previous frame.
func load_game():
	if not FileAccess.file_exists("user://sv.save"):
		return
	else:
		
		var save_gamex = FileAccess.open("sv.save", FileAccess.READ)
